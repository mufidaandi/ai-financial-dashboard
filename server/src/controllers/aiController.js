...
export const testGemini = async (req, res) => {
    try {
        ...
        console.log("- Format check:", process.env.GEMINI_API_KEY.startsWith('AIza') ? 'Valid format' : 'Invalid format');
        ...
        for (const modelName of modelsToTry) {
            try {
                ...
                const result = await genAI.models.generateContent({
                    model: modelName,
                    contents: "Respond with exactly: 'API working'"
                });
                const text = result.text;
        ...
        res.status(500).json({ 
            ...
            suggestions: [
                ...
                "2. Make sure the Gemini API is enabled",
                "3. Check if you need to enable billing",
